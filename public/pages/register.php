...
<section class="min-h-[80vh] flex items-center justify-center px-6 py-16">
  <div class="w-full max-w-sm">
    <div class="text-center mb-8">
      ...
      <p class="text-gray-400 text-sm mt-1">Start converting documents to Braille</p>
    </div>
    ...
    <p class="text-center text-sm text-gray-500 mt-6">
      Already have an account? <a href="<?php echo url('/login'); ?>" class="text-brand-500 hover:text-brand-600 font-medium transition-colors">Sign in</a>
    </p>
  </div>
</section>
...
document.getElementById('register-form').addEventListener('submit', async e => {
  ...
  const res  = await fetch('/api/auth/register', { method: 'POST', body: new FormData(e.target) });
...
renderLayout('Create Account', $content);
